...
<div class="container">
	...
	<form name="policeForm" method="post">
		<div class="input-group">	
			...
			<input type="button" value="검색" onclick="crimeCheck()" class="btn btn-success" />
			<input type="button" value="DB저장" onclick="saveCrimeCheck()" class="btn btn-primary" />
			<input type="button" value="DB삭제" onclick="deleteCrimeCheck()" class="btn btn-danger" />
			<input type="button" value="DB출력" onclick="dbListCrimeCheck()" class="btn btn-warning" />
		</div>
		<div class="input-group">	
	...
	<script>
		...
		const API_KEY = "a1c9d6d2f288aa2fe8dc409de3ac2d069fb0d0573adce4b271d2b9a9233b71d9";
		...
		function deleteCrimeCheck() {
			...
			$.ajax({
				...
				type : 'post',
				data : {year : year},
				success : ()=>alert(year + "년도의 DB자료가 삭제되었습니다."),
				error : ()=>alert("전송오류!")
			});
		}
    ...
    function dbListCrimeCheck() {
		  ...
		  $.ajax({
		    url: 'dbListCrimeCheck',     // 서버에서 만든 매핑
		    type: 'post',
		    data: { year: year },
		    success: function(res) {
		      console.log("DB 데이터:", res);
		      ...
		      res.forEach((item, i) => {
		        str += '<tr>';
		        str += '<td>' + (i+1) + '</td>';
		        str += '<td>' + item.year + '년</td>';
		        str += '<td>' + item.police + '</td>';
		        str += '<td>' + item.robbery + '건</td>';
		        str += '<td>' + item.theft + '건</td>';
		        str += '<td>' + item.murder + '건</td>';
		        str += '<td>' + item.violence + '건</td>';
		        str += '</tr>';
		      });
